...
class Cart extends Component {
    ...
    onComplete=(id)=>{
        console.log(id)
        Axios.patch(`${API_URL}/transactions/${id}`,{status: 'completed'})
        .then((res)=>{
            this.reloadItems()
            this.props.GiveNotif('Transaction is completed')
            setTimeout(()=>{
            ...
            return (
                <Segment key={index} className='cart-product'>
                    <Header className='color-dark' style={{fontSize: '21px'}} block>
                        ...
                        <Label 
                            style={
                                {float: 'unset', fontSize:'15px', margin:'auto'}
                            }
                            className={
                        ...
                        {
                            bill.status==='onprocess'?
                            <Button onClick={()=>{this.onComplete(bill.transactionId)}} style={{float:'right'}} className='bg-sign bg-sign-hvr'>Complete Transaction</Button>
                            : null
                        }
                    </Header>
        ...
        if(this.props.User.role==='admin'){
            ...
            return ( 
                <div style={{padding:'50px 10vw 0 10vw'}}>
                    <Segment>
                        <h2 className='color-dark'>Manage Transactions</h2>
                    </Segment>
